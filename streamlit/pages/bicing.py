...
def bicing():
    ...
    positions = pd.DataFrame(
        [row["coordinates"] for row in stations_data["Location"]],
        columns=["lon", "lat"],
    )
    ...
    st.session_state.radio = st.slider(
        "Please select radio in meters",
        min_value=100,
        max_value=1000,
        step=50,
        value=100,
    ...
    # Make sure we're looking in Barcelona
    ...
    if not near_stations.empty:
        st.session_state.near_station_pos = pd.DataFrame(
            [row["coordinates"] for row in near_stations["Location"]],
            columns=["lon", "lat"],
        )
    ...
    near_stations["distance"] = near_stations.apply(
        lambda x: distance.distance(reference, (x["lat"], x["lon"])).m
    )
